...
# Docker
build:
	docker build --pull -f Dockerfile -t graphql-trial .

drun: build
	@#docker run -v $$(pwd):/app --rm -it graphql-trial bash -c "poetry run python run.py perform"
	docker run -v $$(pwd):/home/appuser --rm -it graphql-trial bash -c "python run.py perform"

dsh: build
	docker run -v $$(pwd):/home/appuser --rm -it graphql-trial poetry shell
